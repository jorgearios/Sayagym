...
if (isset($_GET['salida'])) {
  ...
  echo "<script>window.location='acceso.php?res=salida';</script>";
  exit;
}
  ...
  <div class="container-xl mt-4">
    ...
    <div style="display:grid; grid-template-columns:repeat(3,1fr); gap:14px; margin-bottom:24px;">
      <div class="card" style="text-align:center; padding:20px;">
        <div style="font-size:2.4rem; font-family:'Oswald',sans-serif; font-weight:700; color:var(--text);">
          <?php echo $total_hoy; ?></div>
      ...
      <div class="card" style="text-align:center; padding:20px; border-left:3px solid var(--green);">
        <div style="font-size:2.4rem; font-family:'Oswald',sans-serif; font-weight:700; color:var(--green);">
          <?php echo $dentro_ahora; ?></div>
      ...
      <div class="card" style="text-align:center; padding:20px; border-left:3px solid var(--muted);">
        <div style="font-size:2.4rem; font-family:'Oswald',sans-serif; font-weight:700; color:var(--muted);">
          <?php echo $ya_salieron; ?></div>
          ...
          <table class="gym-table">
            ...
            <tbody>
              <?php if (!$registros || $registros->num_rows === 0): ?>
                <tr>
                  <td colspan="6" style="text-align:center; padding:30px; color:var(--muted);">Sin registros hoy todavía.
                  </td>
                </tr>
              <?php else: ?>
                  ...
                  <tr>
                    <td>
                      <div style="display:flex; align-items:center; gap:10px;">
                        ...
                        <?php else: ?>
                          <div
                            style="width:34px;height:34px;border-radius:50%;background:#F3F4F6;display:flex;align-items:center;justify-content:center;color:#9CA3AF;">
                    ...
                    <td><span class="badge badge-blue"
                        style="font-size:0.7rem;"><?php echo htmlspecialchars($r['medio'] ?? 'Manual'); ?></span></td>
                    <td>
                    ...
                    <td>
                      <?php if (!$r['hora_salida']): ?>
                        <a href="acceso.php?salida=<?php echo $r['id_asistencia']; ?>" class="btn btn-icon"
                          title="Registrar salida" onclick="return confirm('¿Registrar salida ahora?');">
      ...
      <div style="display:flex; flex-direction:column; gap:16px;">
              ...
              <select name="id_socio" class="form-select" required id="sel-socio">
                ...
                <?php while ($s = $socios_activos->fetch_assoc()): ?>
                  <option value="<?php echo $s['id_socio']; ?>">
                    <?php echo htmlspecialchars($s['nombre'] . ' ' . $s['apellido']); ?> (#<?php echo $s['id_socio']; ?>)
                  </option>
                <?php endwhile; ?>
              </select>
            </div>
        ...
        <div class="card" style="border-left:3px solid var(--gold);">
          <div class="card-body" style="padding:16px;">
            <p
              style="font-size:0.78rem; color:#92400E; font-weight:700; text-transform:uppercase; letter-spacing:0.5px; margin-bottom:6px;">
              <i class="ti ti-info-circle me-1"></i> Validación automática
            </p>
            <p style="font-size:0.83rem; color:#4B5563; line-height:1.6;">
              El sistema verifica que el socio tenga membresía vigente antes de registrar la entrada. Socios con
              membresía vencida o cuenta inactiva serán rechazados automáticamente.
            </p>
          </div>
        </div>
      </div>
