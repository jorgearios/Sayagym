 ...
 * Descripción: Herramienta de reseteo de contraseña en caso de olvido.
...
<body>
  <div class="card">
    ...
    <div class="<?php echo $verificado ? 'ok' : 'err'; ?>">
      <?php echo $verificado ? '1' : '0'; ?> password_hash/verify funcionan en este PHP (<?php echo phpversion(); ?>)
    </div>

    <div class="<?php echo $insertado ? 'ok' : 'err'; ?>">
      <?php echo $insertado ? '1' : '0'; ?> Usuario admin
      <?php echo $insertado ? 'creado con ID ' . $insertado : 'NO se pudo crear: ' . $c->error; ?>
    </div>

    <div class="<?php echo $ok_bd ? 'ok' : 'err'; ?>">
      <?php echo $ok_bd ? '1' : '0'; ?> Verificación del hash guardado en BD:
      <?php echo $ok_bd ? 'CORRECTO' : 'FALLA — problema de charset o collation en la BD'; ?>
    </div>
    ...
    <?php else: ?>
      <div class="err" style="margin-top:12px;">
        El hash no se puede guardar/leer correctamente. Revisa el collation de la tabla usuarios (debe ser
        utf8mb4_unicode_ci).
      </div>
    <?php endif; ?>
  </div>
