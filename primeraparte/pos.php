...
if ($_POST && isset($_POST['confirmar_venta'])) {
  ...
  if (empty($ids_prod)) {
    ...
    if ($stock_ok) {
      ...
      echo "<script>window.location='pos.php?venta=$id_venta&res=ok';</script>";
      exit;
    }
  }
}
  ...
  <div class="container-xl mt-4">
    ...
    <div class="row align-items-center mb-4">
      ...
      <div class="col-auto" style="display:flex;gap:10px;align-items:center;">
        <div style="text-align:right;">
          <div style="font-family:'Oswald',sans-serif;font-size:1.4rem;font-weight:700;color:var(--green);">
            $<?php echo number_format($ventas_hoy['t'], 2); ?></div>
    ...
    <?php if ($venta_ok): ?>
      <div class="alert alert-success" style="display:flex;justify-content:space-between;align-items:center;">
        <span>Venta #<?php echo $venta_ok['id_venta']; ?> registrada — Total:
          <strong>$<?php echo number_format($venta_ok['total'], 2); ?></strong></span>
        <a href="ticket.php?id=<?php echo $venta_ok['id_venta']; ?>" target="_blank" class="btn btn-outline"
    ...
    <?php if ($promo_activa): ?>
      <div
        style="background:#DCFCE7;border-left:4px solid var(--green);border-radius:0 6px 6px 0;padding:10px 16px;margin-bottom:16px;font-size:.85rem;color:#15803D;display:flex;align-items:center;gap:10px;">
        ...
        <button type="button"
          onclick="aplicarPromo(<?php echo $promo_activa['tipo'] === 'porcentaje' ? -$promo_activa['valor'] : $promo_activa['valor']; ?>, '<?php echo $promo_activa['tipo']; ?>')"
          class="btn btn-outline"
          style="padding:4px 12px;font-size:.78rem;border-color:var(--green);color:var(--green);">Aplicar</button>
      </div>
    <?php endif; ?>
      ...
      <div class="pos-grid">
          ...
          <div id="grid-productos"
            ...
            while ($p = $productos_activos->fetch_assoc()):
              ...
              <div class="prod-card" data-nombre="<?php echo strtolower(htmlspecialchars($p['nombre'])); ?>"
                ...
                <div class="pstock"><i class="ti ti-package" style="font-size:.8rem;"></i> Stock:
                  <?php echo $p['stock']; ?></div>
              </div>
            <?php endwhile; ?>
          </div>
        </div>
        ...
        <div class="card" style="position:sticky;top:80px;">
          <div class="card-header" style="background:linear-gradient(135deg,var(--red-dark),var(--red));">
            ...
            <button type="button" onclick="limpiarCarrito()" class="btn btn-link"
              style="color:rgba(255,255,255,.7);padding:4px 8px;font-size:.8rem;">Limpiar</button>
          </div>
          <div class="card-body" style="padding:14px;">
            <div id="cart-empty"><i class="ti ti-shopping-cart"
                style="font-size:2rem;display:block;margin-bottom:8px;"></i>Haz clic en un producto para agregarlo</div>
            <div id="cart-items"></div>
            <div id="cart-hidden-inputs"></div>

            <!-- Totales -->
            <div id="cart-totales"
              style="display:none;border-top:2px solid var(--border);margin-top:12px;padding-top:12px;">
              ...
              <div
                style="display:flex;justify-content:space-between;font-size:.88rem;margin-bottom:10px;align-items:center;">
                <span style="color:var(--muted);">Descuento ($)</span>
                <input type="number" name="descuento" id="inp-descuento" min="0" step="0.01" value="0"
                  class="form-control" style="width:110px;text-align:right;padding:6px 8px;" oninput="recalcular()">
              </div>
              <div
                style="display:flex;justify-content:space-between;font-family:'Oswald',sans-serif;font-size:1.5rem;font-weight:700;">
                <span>TOTAL</span>
                ...
                <select name="id_socio" class="form-select">
                  ...
                  <?php while ($s = $socios_lista->fetch_assoc()): ?>
                    <option value="<?php echo $s['id_socio']; ?>">
                      <?php echo htmlspecialchars($s['nombre'] . ' ' . $s['apellido']); ?></option>
                  <?php endwhile; ?>
                </select>
              </div>
              <div style="margin-bottom:10px;">
