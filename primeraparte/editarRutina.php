...
if (!isset($_GET['id'])) { echo "<script>window.location='rutinas.php';</script>"; exit; }
$id_rutina = (int)$_GET['id'];
$rutina = $conexion->query("SELECT * FROM rutinas WHERE id_rutina=$id_rutina")->fetch_assoc();
if (!$rutina) { echo "<script>window.location='rutinas.php';</script>"; exit; }
...
if ($_POST) {
    ...
    echo "<script>window.location='rutinas.php?res=actualizada';</script>"; exit;
}
  ...
  <div class="container-xl mt-4">
    ...
    <form method="POST" class="card" style="max-width:860px; margin:0 auto;">
      <div class="card-header" style="background:linear-gradient(135deg,#4A148C,#7B1FA2);">
        <span class="card-title" style="color:#fff;"><i class="ti ti-edit me-2"></i>Editar Rutina — <?php echo htmlspecialchars($rutina['nombre_rutina']); ?></span>
      </div>
      <div class="card-body">
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr 160px; gap:16px; margin-bottom:24px;">
          <div>
            ...
            <input type="text" name="nombre_rutina" class="form-control" required value="<?php echo htmlspecialchars($rutina['nombre_rutina']); ?>">
          </div>
          <div>
            <label class="form-label">Descripción</label>
            <input type="text" name="descripcion" class="form-control" value="<?php echo htmlspecialchars($rutina['descripcion']); ?>">
          </div>
          <div>
            <label class="form-label">Nivel</label>
            <select name="nivel" class="form-select">
              <?php foreach (['Principiante','Intermedio','Avanzado'] as $nv): ?>
              <option value="<?php echo $nv; ?>" <?php if($rutina['nivel']===$nv) echo 'selected'; ?>><?php echo $nv; ?></option>
              <?php endforeach; ?>
            </select>
          </div>
        </div>
        ...
        <div class="alert alert-danger" style="margin-top:12px;">No hay ejercicios en el catálogo. <a href="ejercicios.php">Agrega primero</a>.</div>
        <?php else: ?>
          ...
          <table style="width:100%; border-collapse:collapse;" id="tabla-ejercicios">
            <thead>
              <tr style="background:#F9FAFB; border-bottom:2px solid var(--border);">
                <th style="padding:10px 12px; font-size:0.78rem; color:var(--muted); text-transform:uppercase; letter-spacing:1px; text-align:left;">#</th>
                <th style="padding:10px 12px; font-size:0.78rem; color:var(--muted); text-transform:uppercase; letter-spacing:1px; text-align:left;">Ejercicio</th>
                <th style="padding:10px 12px; font-size:0.78rem; color:var(--muted); text-transform:uppercase; letter-spacing:1px; width:90px;">Series</th>
                <th style="padding:10px 12px; font-size:0.78rem; color:var(--muted); text-transform:uppercase; letter-spacing:1px; width:90px;">Reps</th>
