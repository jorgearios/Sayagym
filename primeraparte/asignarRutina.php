...
if (isset($_GET['desasignar'])) {
  ...
  echo "<script>window.location='$back&res=desasignado';</script>";
  exit;
}
...
if ($_POST) {
  ...
  echo "<script>window.location='$back&res=asignado';</script>";
  exit;
}
...
$asignaciones = $conexion->query("
    ...
    LEFT JOIN socios s ON sr.id_socio = s.id_socio
    LEFT JOIN rutinas r ON sr.id_rutina = r.id_rutina
    WHERE $where_asig
  ...
  <div class="container-xl mt-4">
    ...
    <?php if (isset($_GET['res'])): ?>
      <div class="alert alert-success">✓
        <?php echo $_GET['res'] === 'asignado' ? 'Rutina asignada correctamente.' : 'Asignación eliminada.'; ?></div>
    <?php endif; ?>
        ...
        <div class="table-responsive">
          <table class="gym-table">
            <thead>
              <tr>
                <?php if ($id_rutina): ?>
                  <th>Socio</th><?php else: ?>
                  <th>Rutina</th><?php endif; ?>
                <th>Inicio</th>
                <th>Fin</th>
                <th>Estado</th>
                <th>Acción</th>
            ...
            <tbody>
              <?php if (!$asignaciones || $asignaciones->num_rows === 0): ?>
                <tr>
                  <td colspan="5" style="text-align:center; padding:30px; color:var(--muted);">Sin asignaciones aún.</td>
                </tr>
              <?php else: ?>
                <?php while ($a = $asignaciones->fetch_assoc()):
                  ...
                  <tr>
                    <td class="td-name">
                      <?php echo $id_rutina ? htmlspecialchars($a['nombre'] . ' ' . $a['apellido']) : htmlspecialchars($a['nombre_rutina']); ?>
                    </td>
                    <td class="td-muted"><?php echo date('d/m/Y', strtotime($a['fecha_inicio'])); ?></td>
                    <td class="td-muted"><?php echo $a['fecha_fin'] ? date('d/m/Y', strtotime($a['fecha_fin'])) : '—'; ?>
                    </td>
                    <td><span
                        class="badge <?php echo $activa ? 'badge-green' : 'badge-gray'; ?>"><?php echo $activa ? 'Activa' : 'Terminada'; ?></span>
                    </td>
                    <td>
            ...
            <div>
              ...
              <select name="id_socio" class="form-select" required>
                ...
                while ($s = $socios_lista->fetch_assoc()):
                  ...
                  ?>
                  <option value="<?php echo $s['id_socio']; ?>" <?php echo $sel; ?>>
            ...
            <div>
              ...
              <select name="id_rutina" class="form-select" required>
                ...
                while ($r2 = $rutinas_lista->fetch_assoc()):
                  ...
                  ?>
                  <option value="<?php echo $r2['id_rutina']; ?>" <?php echo $sel; ?>>
                    <?php echo htmlspecialchars($r2['nombre_rutina']); ?> (<?php echo $r2['nivel']; ?>)</option>
                <?php endwhile; ?>
              </select>
            </div>
            <div>
