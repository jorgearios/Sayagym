...
if (isset($_GET['eliminar'])) {
    ...
    echo "<script>window.location='promociones.php?res=eliminada';</script>"; exit;
}
...
if (isset($_GET['toggle'])) {
    ...
    echo "<script>window.location='promociones.php';</script>"; exit;
}
...
if ($_POST) {
    ...
    echo "<script>window.location='promociones.php?res=guardada';</script>"; exit;
}
...
<div class="page-wrapper">
  <div class="container-xl mt-4">
    ...
    <div class="alert alert-success">✓ Promoción <?php echo $_GET['res'] === 'eliminada' ? 'eliminada.' : 'guardada correctamente.'; ?></div>
    <?php endif; ?>
    ...
    <div style="display:grid;grid-template-columns:1fr 360px;gap:20px;align-items:start;">
      ...
      <!-- LISTA ────────────────────────────────────────── -->
      <div class="card">
        <div class="card-header gray"><h3 class="card-title">Promociones registradas</h3></div>
        <div class="table-responsive">
          <table class="gym-table">
            <thead>
              <tr><th>Nombre</th><th>Tipo</th><th>Descuento</th><th>Vigencia</th><th>Estado</th><th>Acciones</th></tr>
            </thead>
            <tbody>
              <?php if ($promos->num_rows === 0): ?>
              <tr><td colspan="6" style="text-align:center;padding:30px;color:var(--muted);">Sin promociones. Crea la primera.</td></tr>
              <?php endif; ?>
              ...
              <tr>
                ...
                <td class="td-muted"><?php echo date('d/m/Y',strtotime($p['fecha_inicio'])); ?> – <?php echo date('d/m/Y',strtotime($p['fecha_fin'])); ?></td>
                <td>
                  <span class="badge <?php echo $vigente ? 'badge-green' : ($p['estado']==='activa' ? 'badge-gold' : 'badge-gray'); ?>">
                    <?php echo $vigente ? 'VIGENTE' : strtoupper($p['estado']); ?>
                  </span>
                </td>
                <td>
                  <div class="btn-list">
                    <a href="promociones.php?editar=<?php echo $p['id_promo']; ?>" class="btn btn-icon edit" title="Editar"><i class="ti ti-edit"></i></a>
                    <a href="promociones.php?toggle=<?php echo $p['id_promo']; ?>" class="btn btn-icon" title="<?php echo $p['estado']==='activa'?'Desactivar':'Activar'; ?>">
                      <i class="ti ti-<?php echo $p['estado']==='activa'?'eye-off':'eye'; ?>"></i>
                    </a>
                    <a href="promociones.php?eliminar=<?php echo $p['id_promo']; ?>" class="btn btn-icon" title="Eliminar" onclick="return confirm('¿Eliminar esta promoción?');"><i class="ti ti-trash"></i></a>
                  </div>
                </td>
              </tr>
              <?php endwhile; ?>
          ...
          <div style="display:flex;flex-direction:column;gap:12px;">
            ...
            <div>
              ...
              <select name="tipo" class="form-select" id="sel-tipo" onchange="cambiarTipo()">
                <option value="porcentaje" <?php if(!$edit || $edit['tipo']==='porcentaje') echo 'selected'; ?>>Porcentaje (%)</option>
                <option value="monto_fijo" <?php if($edit && $edit['tipo']==='monto_fijo') echo 'selected'; ?>>Monto fijo ($)</option>
              </select>
            </div>
            <div>
            ...
            <div>
              ...
              <select name="estado" class="form-select">
                <option value="activa"   <?php if(!$edit || $edit['estado']==='activa')   echo 'selected'; ?>>Activa</option>
                <option value="inactiva" <?php if($edit && $edit['estado']==='inactiva') echo 'selected'; ?>>Inactiva</option>
              </select>
            </div>
          </div>
        </div>
